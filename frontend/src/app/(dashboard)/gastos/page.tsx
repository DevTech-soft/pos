  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="bg-[#121927] border border-rose-500/20 rounded-2xl p-5">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <label className="text-[11px] font-bold text-[#7E8CA6] uppercase tracking-[0.15em] block mb-2">Descripción</label>
              <input value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="..."
                className="w-full bg-[#1A2333] border border-[#2A3650] rounded-xl px-4 py-2.5 text-[14px] text-[#F3F6FA] outline-none" />
            </div>
            <div>
              <label className="text-[11px] font-bold text-[#7E8CA6] uppercase tracking-[0.15em] block mb-2">Monto</label>
              <input type="number" value={form.amount} onChange={e => setForm(p => ({ ...p, amount: e.target.value }))} placeholder="0"
                className="w-full bg-[#1A2333] border border-[#2A3650] rounded-xl px-4 py-2.5 text-[14px] text-[#F3F6FA] outline-none" />
            </div>
            <div>
              <label className="text-[11px] font-bold text-[#7E8CA6] uppercase tracking-[0.15em] block mb-2">Fecha</label>
      ...
      <div className="space-y-3">
        {expenses.length === 0 && <p className="text-[#7E8CA6] text-center py-8">No hay registros</p>}
        {expenses.map(e => (
          <div key={e.id} className={`bg-[#121927] border rounded-2xl p-4 flex items-center justify-between ${e.type === 'EGRESO' ? 'border-[#2A3650]' : 'border-emerald-500/20'}`}>
            <div>
              <p className="text-[14px] font-medium text-[#F3F6FA]">{e.description}</p>
              <p className="text-[12px] text-[#7E8CA6] mt-0.5">{e.category} · {formatDate(e.date)}</p>
            </div>
