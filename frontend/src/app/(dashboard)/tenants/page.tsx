    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      {showForm && (
        <div className="bg-[#101520] border border-amber-500/20 rounded-2xl p-5">
          <h2 className="text-[16px] font-semibold text-[#EDF2F7] mb-4">Registrar piscina</h2>
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <input value={(form as any)[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))}
                  ...
                  className="w-full bg-[#141B28] border border-[#1C2535] rounded-xl px-4 py-2.5 text-[14px] text-[#EDF2F7] placeholder:text-[#2A3A52] outline-none focus:border-amber-500/50" />
              </div>
            ))}
          </div>
      ...
      <div className="grid lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {tenants.map(t => (
          <div key={t.id} className={`bg-[#101520] border rounded-2xl p-5 ${t.isActive ? 'border-[#1C2535]' : 'border-rose-500/20 opacity-70'}`}>
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[15px] font-semibold text-[#EDF2F7]">{t.name}</p>
                  <p className="text-[11px] text-[#4A5568] mt-0.5">{t.id.slice(0, 8)}...</p>
                </div>
              </div>
              <button
                onClick={() => toggle.mutate({ id: t.id, isActive: !t.isActive })}
                className="text-[#4A5568] hover:text-amber-400 transition-colors"
            ...
            <div className="space-y-1 text-[13px]">
              {t.address && <p className="text-[#4A5568]">📍 {t.address}</p>}
              {t.phone && <p className="text-[#4A5568]">📞 {t.phone}</p>}
              {t.email && <p className="text-[#4A5568]">✉️ {t.email}</p>}
            </div>
          </div>
        ))}
      </div>
